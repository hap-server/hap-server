...
const native_hook: import('../native-hook').NativeHook | null = /* global.__HAP_SERVER_NATIVE_HOOK__ ? global.__HAP_SERVER_NATIVE_HOOK__({
    InternalSymbols,
    // PluginManager,
    Client,
    Connection,
...
// const modals = native_hook && native_hook.Modals ? new native_hook.Modals<typeof ModalsComponent>(client) :
//     new Modals<typeof ModalsComponent>();
// modals.component = ModalsComponent;
// // modals.i18n = i18n;
const client = new Client(undefined, undefined, ServiceTileComponents);
const modals = new Modals<typeof ModalsComponent, React.Component & {show: boolean}>();
modals.component = ModalsComponent;
modals.i18n = i18n;
...
function setAssetToken(token: string | null) {
    document.cookie = 'asset_token=' +
        (token ? encodeURIComponent(token) : '; expires=Thu, 01 Jan 1970 00:00:00 GMT') +
        '; path=/' + (location.protocol === 'https:' ? '; Secure' : '');

    // else document.cookie = 'asset_token=; expires=Thu, 01 Jan 1970 00:00:00 GMT';
}
...
//     render() {
//         return <TranslationContext.Provider value={i18n}>
//             <App native_hook={native_hook} client={client} modals={modals} getAssetURL={getAssetURL}
//                 errors={errors} setAssetToken={setAssetToken} />
