<template>
    <transition @after-leave="$emit('close')">
        <div v-if="show" class="accessory-details-wrapper" :data-accessory-uuid="service.accessory.uuid" :data-service-uuid="service.uuid" :data-service-type="service.type" @click="close">
            <component v-if="component" :is="component" ref="service" :service="service" @show-settings="$emit('show-settings')" @show-accessory-settings="$emit('show-accessory-settings')" />
            ...
            <accessory-details v-else class="unsupported-service" :name="service.name || service.accessory.name" @show-settings="$emit('show-accessory-settings')">
