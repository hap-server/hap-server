...
// Module will be patched to add a virtual "@hap-server/api" module
// "@hap-server/api/storage" is a node-persist instance (will be configured and initialised automatically, before
// anything the plugin registers is used, but *after running this file*)
// "@hap-server/api/hap" is hap-nodejs
...
import globalconfig = require('@hap-server/api/plugin-config');
...
async function createAccessory(name: string, config: any, __log: typeof log) {
    ...
    // Add the Switch service
    const switch_service = accessory.addService(Service.Switch, name);

    switch_service.getCharacteristic(Characteristic.On)
        .onSet(async value => {
            await storage.setItem('VirtualSwitch.' + switch_uuid, value);

            _log.info('Switch %s is now %s', name, value ? 'on' : 'off');
        })
        .updateValue(initial_state);

    _log.info('Switch %s is %s', name, initial_state ? 'on' : 'off');
...
hapserver.registerDynamicAccessoryPlatform('VirtualSwitches', async (accessory_platform, config) => {
    ...
    const switches = await Promise.all((config.switches as string[]).map(name => createAccessory(name, config, __log)));
    ...
    accessory_platform.onreload = async function(new_config) {
        ...
        for (const accessory of switches) {
            ...
            const switch_name = (new_config.switches as string[])
...
const accessory_setup = new AccessorySetup('VirtualSwitch', (data, connection) => {
    //
});
