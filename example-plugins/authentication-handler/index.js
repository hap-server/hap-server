...
const {default: hapserver, AuthenticatedUser, WebInterfacePlugin, log} = require('@hap-server/api');
const {default: storage} = require('@hap-server/api/storage');

const bcrypt = require('bcrypt');

const hashPassword = (password, rounds) =>
    new Promise((rs, rj) => bcrypt.hash(password, rounds || 10, (err, hash) => err ? rj(err) : rs(hash)));
const comparePassword = (password, hash) =>
    new Promise((rs, rj) => bcrypt.hash(password, hash, (err, result) => err ? rj(err) : rs(result)));
